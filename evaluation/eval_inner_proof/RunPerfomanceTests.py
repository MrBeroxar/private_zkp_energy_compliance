...
class RunPerformanceTests(ZokCommandRunner):
    ...
    def run_tests(self):
        ...
        for subfolder in self.subfolders:
            ...
            metrics = {"type": subfolder}

            commands = [
                ("compile", "zokrates compile -i test.zok -c bls12_377"),
                ("setup", "zokrates setup -b ark -s gm17"),
                (
                    "compute-witness",
                    "zokrates compute-witness --abi --stdin < input.json",
                ),
                ("generate-proof", "zokrates generate-proof -b ark -s gm17"),
            ...
            for command_name, command in commands:
                ...
                if returncode != 0:
                    print(f"Command failed in {subfolder}: {command}")  # Debugging
    ...
    def export_to_csv(self, filename="32_sig.csv"):
